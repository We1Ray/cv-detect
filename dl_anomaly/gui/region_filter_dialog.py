...
from dl_anomaly.core.region_ops import region_to_display_image, select_shape
...
class RegionFilterDialog(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        self._min_val_entry.pack(side=tk.LEFT, padx=(0, 4))
        self._min_val_entry.bind("<Return>", lambda _e: self._on_min_entry_commit())
        self._min_val_entry.bind("<FocusOut>", lambda _e: self._on_min_entry_commit())
        ...
        self._max_val_entry.pack(side=tk.LEFT, padx=(0, 4))
        self._max_val_entry.bind("<Return>", lambda _e: self._on_max_entry_commit())
        self._max_val_entry.bind("<FocusOut>", lambda _e: self._on_max_entry_commit())
    ...
    def _on_feature_selection_changed(self) -> None:
        ...
        self._min_val_var.set(self._format_feature_value(lo, feat))
        self._max_val_var.set(self._format_feature_value(hi, feat))
    ...
    def _on_min_slider(self, _val: str) -> None:
        ...
        self._min_val_var.set(self._format_feature_value(real_val, feat))
    ...
    def _on_max_slider(self, _val: str) -> None:
        ...
        if pos < min_pos:
            ...
            pos = min_pos
        real_val = self._slider_to_value(pos, feat)
        self._max_val_var.set(self._format_feature_value(real_val, feat))

    # ------------------------------------------------------------------ #
    #  Entry → Slider sync                                                #
    # ------------------------------------------------------------------ #

    def _on_min_entry_commit(self) -> None:
        """User typed a value in the min entry — update the slider."""
        feat = self._current_feature_attr()
        try:
            value = float(self._min_val_var.get())
        except ValueError:
            return
        lo, hi = self._feature_ranges.get(feat, (0.0, 1.0))
        value = max(lo, min(hi, value))
        slider_pos = self._value_to_slider(value, feat)
        self._min_slider.set(slider_pos)
        self._min_val_var.set(self._format_feature_value(value, feat))

    def _on_max_entry_commit(self) -> None:
        """User typed a value in the max entry — update the slider."""
        feat = self._current_feature_attr()
        try:
            value = float(self._max_val_var.get())
        except ValueError:
            return
        lo, hi = self._feature_ranges.get(feat, (0.0, 1.0))
        value = max(lo, min(hi, value))
        slider_pos = self._value_to_slider(value, feat)
        self._max_slider.set(slider_pos)
        self._max_val_var.set(self._format_feature_value(value, feat))
    ...
    def _accept(self) -> None:
        ...
        if self._on_accept is not None and self._filtered_region is not None:
            ...
            disp = self._filtered_region.to_binary_mask()

            # Build conditions list for recipe metadata
            conditions_list = [
                {"feature": c.feature, "min_val": c.min_val, "max_val": c.max_val}
                for c in self._conditions
            ]

            self._on_accept(self._filtered_region, disp, name, conditions_list)
