"""
core/vm_preprocessor.py - 影像前處理模組
...
class ImagePreprocessor:
    ...
    def load_image(self, path: Union[str, Path]) -> np.ndarray:
        ...
        if image.ndim < 2 or image.ndim > 3:
            raise IOError(
                f"載入的影像維度不正確 (ndim={image.ndim})，預期 2D 或 3D: {path}"
            )
    ...
    def resize(self, image: np.ndarray) -> np.ndarray:
        ...
        Returns:
            縮放後的影像。
        """
        if self.config.target_width <= 0 or self.config.target_height <= 0:
            raise ValueError(
                f"目標尺寸必須為正整數，收到 width={self.config.target_width}, "
                f"height={self.config.target_height}"
            )
