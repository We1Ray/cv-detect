...
class VariationModel:
    ...
    def train_incremental(self, image: np.ndarray) -> None:
        ...
        if self._mean is not None and img.shape != self._mean.shape:
            raise ValueError(
                f"影像形狀 {img.shape} 與模型形狀 {self._mean.shape} 不符"
            )
    ...
    def prepare(self, abs_threshold: int = 10, var_threshold: float = 3.0) -> None:
        ...
        if abs_threshold < 0:
            raise ValueError(f"abs_threshold 必須 >= 0，收到 {abs_threshold}")
        if var_threshold < 0:
            raise ValueError(f"var_threshold 必須 >= 0，收到 {var_threshold}")
