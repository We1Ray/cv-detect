...
class Inspector:
    ...
    def compare(self, image: np.ndarray) -> InspectionResult:
        ...
        """
        if image.shape[:2] != self._mean.shape[:2]:
            raise ValueError(
                f"影像形狀 {image.shape[:2]} 與模型形狀 {self._mean.shape[:2]} 不符"
            )
    ...
    def compare_multiscale(
        ...
        """
        if levels < 1:
            raise ValueError(f"levels 必須 >= 1，收到 {levels}")
