...
def _wrap_cv2_error(func_name: str, err: cv2.error) -> None:
    """Log an OpenCV error with context and re-raise as ``RuntimeError``."""
    msg = f"{func_name}: OpenCV error - {err}"
    logger.error(msg)
...
def exp_image(img: np.ndarray, base: str = "e") -> np.ndarray:
    """Exponential gray-value transform."""
...
def gamma_image(img: np.ndarray, gamma: float = 1.0) -> np.ndarray:
    ...
    validate_image(img, "gamma_image/img")
...
def mean_image(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    """Mean (box) filter."""
    validate_image(img, "mean_image/img")
    k = _odd(ksize)
...
def median_image(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    ...
    validate_image(img, "median_image/img")
    k = _odd(ksize)
...
def gauss_filter(img: np.ndarray, sigma: float = DEFAULT_SIGMA) -> np.ndarray:
    ...
    validate_image(img, "gauss_filter/img")
...
def gauss_blur(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    ...
    validate_image(img, "gauss_blur/img")
    k = _odd(ksize)
...
def bilateral_filter(
    ...
    validate_image(img, "bilateral_filter/img")
    try:
        return cv2.bilateralFilter(img, d, sigma_color, sigma_space)
    except cv2.error as exc:
        _wrap_cv2_error("bilateral_filter", exc)
...
def sobel_filter(img: np.ndarray, direction: str = "both") -> np.ndarray:
    ...
    """
    validate_image(img, "sobel_filter/img")
    gray = _ensure_gray(img)
    if direction == "x":
...
def prewitt_filter(img: np.ndarray) -> np.ndarray:
    ...
    validate_image(img, "prewitt_filter/img")
...
def edges_canny(
    ...
    validate_image(img, "edges_canny/img")
    gray = _ensure_gray(img)
    try:
        if sigma > 0:
            k = _ksize_from_sigma(sigma)
        ...
        _wrap_cv2_error("edges_canny", exc)
...
def gray_erosion(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    ...
    validate_image(img, "gray_erosion/img")
...
def gray_dilation(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    ...
    validate_image(img, "gray_dilation/img")
...
def gray_opening(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    ...
    validate_image(img, "gray_opening/img")
    return cv2.morphologyEx(img, cv2.MORPH_OPEN, _morph_kernel(ksize))


def gray_closing(img: np.ndarray, ksize: int = DEFAULT_KSIZE) -> np.ndarray:
    """Grayscale morphological closing (dilation then erosion)."""
    validate_image(img, "gray_closing/img")
    return cv2.morphologyEx(img, cv2.MORPH_CLOSE, _morph_kernel(ksize))


def top_hat(img: np.ndarray, ksize: int = 5) -> np.ndarray:
    """Top-hat (white hat): ``img - opening(img)``."""
    return cv2.morphologyEx(img, cv2.MORPH_TOPHAT, _morph_kernel(ksize))


def bottom_hat(img: np.ndarray, ksize: int = 5) -> np.ndarray:
    """Bottom-hat (black hat): ``closing(img) - img``."""
    return cv2.morphologyEx(img, cv2.MORPH_BLACKHAT, _morph_kernel(ksize))
...
def dyn_threshold(
    ...
    validate_image(img_light, "dyn_threshold/img_light")
    validate_image(img_dark, "dyn_threshold/img_dark")
...
def var_threshold(
    ...
    validate_image(img, "var_threshold/img")
    gray = _ensure_gray(img).astype(np.float64)
    kw = _odd(width)
    kh = _odd(height)
...
def local_threshold(
    ...
    validate_image(img, "local_threshold/img")
    gray = _ensure_gray(img).astype(np.float64)
    k = _odd(ksize)
...
def gen_gauss_filter(shape: Tuple[int, int], sigma: float) -> np.ndarray:
...
def freq_filter(img: np.ndarray, filter_type: str = "lowpass", cutoff: float = 30.0) -> np.ndarray:
...
def entropy_image(img: np.ndarray, ksize: int = 5) -> np.ndarray:
    ...
    For each pixel neighbourhood of size ``ksize x ksize``, a local
    histogram is built and Shannon entropy is computed.  The result is
    ...
    for row in range(h):
        for col in range(w):
            patch = padded[row : row + k, col : col + k].ravel().astype(np.uint8)
            hist = np.bincount(patch, minlength=256).astype(np.float64)
            hist = hist / hist.sum()
            nonzero = hist[hist > 0]
...
def template_match_ncc(
    ...
    """
    validate_image(img, "template_match_ncc/img")
    validate_image(template, "template_match_ncc/template")
...
def hough_lines(
    ...
    """
    validate_image(img, "hough_lines/img")
    gray = _ensure_gray(img)
    try:
        ...
        _wrap_cv2_error("hough_lines", exc)
    if img.ndim == 2:
        out = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    else:
        out = img.copy()
...
def hough_circles(
    ...
    """
    validate_image(img, "hough_circles/img")
    gray = _ensure_gray(img)
    try:
        ...
        _wrap_cv2_error("hough_circles", exc)
    if img.ndim == 2:
        out = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    else:
        out = img.copy()
