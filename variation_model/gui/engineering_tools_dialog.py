...
class EngineeringToolsDialog(tk.Toplevel):
    ...
    def _calibrate_known_distance(self) -> None:
        try:
            from core.calibration import calibrate_from_known_distance
        except ImportError:
            messagebox.showerror("錯誤", "無法載入 calibration 模組。", parent=self)
            return
    ...
    def _calibrate_from_chessboard(self) -> None:
        ...
        if img is None:
            return
        try:
            from core.calibration import calibrate_from_chessboard
        except ImportError:
            messagebox.showerror("錯誤", "無法載入 calibration 模組。", parent=self)
            return
    ...
    def _save_calibration(self) -> None:
        if self._calib_result is None:
            ...
            return
        try:
            from core.calibration import save_calibration
        except ImportError:
            messagebox.showerror("錯誤", "無法載入 calibration 模組。", parent=self)
            return
    ...
    def _undistort_image(self) -> None:
        ...
        if self._calib_result is None:
            ...
            return
        try:
            from core.calibration import undistort_image
            import cv2
        except ImportError:
            messagebox.showerror("錯誤", "無法載入 calibration 模組。", parent=self)
            return
    ...
    def _show_world_grid(self) -> None:
        ...
        if self._world_mapping is None:
            ...
            return
        try:
            from core.calibration import draw_world_grid
            import cv2
        except ImportError:
            messagebox.showerror("錯誤", "無法載入 calibration 模組。", parent=self)
            return
    ...
    def _create_inspection_pipeline(self) -> None:
        try:
            from core.parallel_pipeline import create_inspection_pipeline
        except ImportError:
    ...
    def _create_batch_processor(self) -> None:
        try:
            from core.parallel_pipeline import create_batch_processor
        except ImportError:
            messagebox.showerror(
                "錯誤", "無法載入 parallel_pipeline 模組。", parent=self,
            )
    ...
    def _benchmark_pipeline(self) -> None:
        ...
        try:
            from core.parallel_pipeline import benchmark_pipeline
        except ImportError:
            messagebox.showerror(
                "錯誤", "無法載入 parallel_pipeline 模組。", parent=self,
            )
    ...
    def _stitch_from_dir(self) -> None:
        ...
        try:
            from core.stitching import stitch_from_directory
            import cv2
        except ImportError:
            messagebox.showerror(
                "錯誤", "無法載入 stitching 模組。", parent=self,
    ...
    def _crop_borders(self) -> None:
        ...
        try:
            from core.stitching import crop_black_borders
            import cv2
        except ImportError:
            messagebox.showerror(
                "錯誤", "無法載入 stitching 模組。", parent=self,
