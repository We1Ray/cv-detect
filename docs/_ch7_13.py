...
def write_ch7(p):
    ...
    # 7.2
    p.sec_title("基於形狀的匹配")
    p.txt("基於形狀的匹配使用邊緣/梯度資訊而非原始像素值，對照明變化和輕微表面變化更具魯棒性。這是 Halcon find_shape_model 使用的方法。\n\n"
          "流程：\n"
          "1. 從參考零件建立形狀模型（提取邊緣）\n"
          "2. 對每個測試影像搜尋形狀模型\n"
...
def write_ch11(p):
    ...
    p.txt("變異模型是 Halcon 等系統使用的統計方法。利用多張訓練影像計算每個位置像素值的均值和標準差來建模。檢測時，顯著偏離模型的像素被標記為瑕疵。")
